 ...
 * Time: 7:19 PM
 */
public class BachStepsDataDriven {
    private static Set<Long> threads;

    public int register() {
        synchronized (BachTestDataDriven.class) {
            if (BachStepsDataDriven.threads == null) {
                BachStepsDataDriven.threads = new HashSet<>();
            }
            BachStepsDataDriven.threads.add(Thread.currentThread().getId());
            return BachStepsDataDriven.threads.size();
        }
    }
...
/*
    ...
    public void then_threads_count_should_be(int value) {
        assertThat(BachStepsDataDriven.threads.size()).isGreaterThan(value);
    }
}
